...
const happy=Intl.DateTimeFormat().resolvedOptions().timeZone;
console.log(happy);


    // var offset = new Date().getTimezoneOffset(), o = Math.abs(offset);
    // const g=(offset < 0 ? "+" : "-") + ("00" + Math.floor(o / 60)).slice(-2) + ":" + ("00" + (o % 60)).slice(-2);
    // console.log(g);
    // document.querySelector(".o").innerHTML=`${happy} (UTC${g})`;
    ...
    for (var i = 0; i < select.length; i++){
      ...
      if(option==happy)
      {
        document.getElementById('timezone').getElementsByTagName('option')[i].selected = 'selected';
